 ...
 */
public class HillClimbingSearchFlat2 extends NodeExpander implements Search {
	...
	private int pasosLlano=0;
	
	private int pasosMalos=0;
	 ...
	 */
	public HillClimbingSearchFlat2(HeuristicFunction hf) {
		this.hf = hf;
	}
	...
	public List<Action> search(Problem p) throws Exception {
		...
		while (!CancelableThread.currIsCanceled()) {
			...
			if ((neighbor == null) || (getValue(neighbor) <= getValue(current))) {
				...
				for(int i=0;i<N;i++){
					if((anterior[i]!=null && anterior[i].getState().equals(neighbor.getState()))){
